...
Partial pooling across country segments did not reduce prediction error for the data-sparse markets (Germany, France) relative to the pooled model (Table~\ref{tab:country_mae}). The tight between-segment prior required to keep the small-segment posterior geometry tractable shrinks the segment parameters strongly toward the global values, leaving the hierarchical and pooled models almost indistinguishable; and the pooled model is already accurate in these segments. The borrowing-of-strength mechanism is therefore not contradicted, but the country segments in this dataset are not heterogeneous enough, relative to their sample sizes, for it to deliver a measurable gain.

\paragraph{RQ3 / H3: Decision-theoretic targeting.}
Targeting by the posterior probability $P(\text{CLV} > c)$ did not outperform point-estimate targeting in cumulative realised value (Table~\ref{tab:targeting_sim_bg_nbd_bayesian}), and this ordering was robust across the swept intervention costs. The finding is instructive rather than merely negative: it shows that the decision value of the full posterior is contingent on the objective. For maximising expected captured value, ranking by the posterior mean is close to optimal and the extra information in the posterior tails is not needed; the probability rule instead encodes a risk-averse preference that would pay off under a loss-sensitive utility or a hard budget constraint. The posterior is thus a means to whichever decision rule the objective calls for, not a guaranteed improvement on every rule.
