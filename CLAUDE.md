...
**Versioned output convention:** each round of edits produces an incrementing `texts/thesis_vN.pdf` (v1, v2, v3, …). `build_thesis.sh` auto-bumps to the next unused number. Do not overwrite an existing version. As of the last session: **v4 is the last built PDF (59 pages)** — v1 = first clean build, v2 = em-dash/prose-polish pass, v3 = supervisor-feedback revisions, v4 = first build with real pipeline results (Chapter 5/6 filled from `outputs/`, methodology reconciled).

> **⚠ CURRENT STATUS (2026-07-05) — read `PROJECT_CONTEXT.md` §0 first.** After v4, an **H2/H3 analysis redesign** was implemented in code + data (committed, `outputs/` regenerated) but the **thesis prose is not yet updated and v5 is not built**. Next session must: rewrite the ch5/ch6 H2 section (verdict → *Partially supported*, three-way none/partial/complete pooling comparison — no-pooling column now in `tab:country_mae`) and H3 section (predictive-CLV targeting: headline shortfall −1.7% not −55.6%, near-parity, plus the expected-vs-predictive-probability pitfall finding), update `tab:verdicts`, then `./build_thesis.sh` → `thesis_v5.pdf`. Exact numbers and the step list are in `PROJECT_CONTEXT.md` §0. The standard models use **pymc-marketing** (do not revive the hand-rolled BG/NBD likelihood); the working env is the Python 3.11 venv at `~/.venvs/bayesclv`.
